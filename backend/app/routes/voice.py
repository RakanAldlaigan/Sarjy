...
from fastapi import APIRouter, BackgroundTasks, Depends, Form, Response, UploadFile
...
async def chat(
    ...
    if session_id and session_service.touch_session(session_id, user_id):
        background_tasks.add_task(session_service.update_last_active, session_id, user_id)
    else:
    ...
    try:
        ...
        message_service.save_message(session_id, user_id, "user", transcript)
        ...
        pending_action, note_draft = session_service.get_session_turn_state(session_id, user_id)
        ...
        background_tasks.add_task(message_service.save_message, session_id, user_id, "assistant", reply)
...
def handle_pending_action(body: PendingActionRequest, user_id: str = Depends(get_current_user_id)):
    ...
    message_service.save_message(body.session_id, user_id, "assistant", reply)
    try:
        audio_reply = elevenlabs_service.synthesize_speech(reply)
    except Exception:
        audio_reply = b""
